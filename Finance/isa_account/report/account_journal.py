...
class journal_print_isa(report_sxw.rml_parse, common_report_header):

    def __init__(self, cr, uid, name, context=None):
        if context is None:
            context = {}
        super(journal_print_isa, self).__init__(cr, uid, name, context=context)
        self.period_ids = []
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        return super(journal_print_isa, self).set_context(objects, data, ids, report_type=report_type)
    ...
    def lines(self, data, period_id, journal_id=False,):
        if not journal_id:
            journal_id = self.journal_ids
        else:
            journal_id = [journal_id]
    ...
    def _get_fiscalyear(self, data):
        ...
        return super(journal_print_isa, self)._get_fiscalyear(data)

    def _get_account(self, data):
        if data['model'] == 'account.journal.period':
            return self.pool.get('account.journal.period').browse(self.cr, self.uid, data['id']).company_id.name
        return super(journal_print_isa, self)._get_account(data)
...
report_sxw.report_sxw('report.account.journal.period.print.isa', 'account.journal.period', 'isa_account/report/account_journal_isa.rml', parser=journal_print_isa, header='external')
report_sxw.report_sxw('report.account.journal.period.print.sale.purchase.isa', 'account.journal.period', 'isa_account/report/account_journal_sale_purchase_isa.rml', parser=journal_print_isa, header='external')
