...
class partner_balance(report_sxw.rml_parse, common_report_header):
    ...
    def lines(self,data):
        ...
        if data['form']['report_currency']=='usd':
            self.cr.execute(
                "SELECT p.ref,l.account_id,ac.name AS account_name,ac.code AS code,p.name, sum(debit_usd) AS debit, sum(credit_usd) AS credit, " \
                        "CASE WHEN sum(debit_usd) > sum(credit_usd) " \
                            "THEN sum(debit_usd) - sum(credit_usd) " \
                            "ELSE 0 " \
                        "END AS sdebit, " \
                        "CASE WHEN sum(debit_usd) < sum(credit_usd) " \
                            "THEN sum(credit_usd) - sum(debit_usd) " \
                            "ELSE 0 " \
                        "END AS scredit, " \
                        "(SELECT sum(debit_usd-credit_usd) " \
...
report_sxw.report_sxw('report.account.partner.balance.isa', 'res.partner', 'isa_account/report/account_partner_balance.rml',parser=partner_balance, header="internal")
