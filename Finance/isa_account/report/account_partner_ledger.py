...
#                    "AND " + self.query +" " \
    ...
    def lines(self, partner,data):
        ...
        if data['form']['report_currency']=='base':
            ...
            self.cr.execute(
            "SELECT l.id, l.date, j.code, acc.code as a_code, acc.name as a_name, l.ref, m.name as move_name, l.name, l.debit_usd, l.credit_usd, l.amount_currency,l.currency_id, c.symbol AS currency_code " \
            "FROM account_move_line l " \
            "LEFT JOIN account_journal j " \
                "ON (l.journal_id = j.id) " \
            "LEFT JOIN account_account acc " \
    ...
    def _get_intial_balance(self, partner,data):
        ...
        if data['form']['report_currency']=='base':
            ...
            self.cr.execute(
            "SELECT COALESCE(SUM(l.debit_usd),0.0), COALESCE(SUM(l.credit_usd),0.0), COALESCE(sum(debit_usd-credit_usd), 0.0) " \
            "FROM account_move_line AS l,  " \
            "account_move AS m "
            "WHERE l.partner_id = %s " \
            "AND m.id = l.move_id " \
...
report_sxw.report_sxw('report.account.third_party_ledger.isa', 'res.partner',
        'isa_account/report/account_partner_ledger.rml',parser=third_party_ledger,
        header='internal')

report_sxw.report_sxw('report.account.third_party_ledger_other.isa', 'res.partner',
        'isa_account/report/account_partner_ledger_other.rml',parser=third_party_ledger,
        header='internal')
