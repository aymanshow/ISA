...
class journal_print(report_sxw.rml_parse, common_report_header):
    ...
    def lines(self, period_id,data):
        ...
        if data['form']['report_currency']=='base':
            ...
            self.cr.execute('SELECT j.code, j.name, l.amount_currency,c.symbol AS currency_code,l.currency_id, '
                        'SUM(l.debit_usd) AS debit, SUM(l.credit_usd) AS credit '
                        'FROM account_move_line l '
                        'LEFT JOIN account_move am ON (l.move_id=am.id) '
                        'LEFT JOIN account_journal j ON (l.journal_id=j.id) '
                        'LEFT JOIN res_currency c on (l.currency_id=c.id)'
    ...
    def _sum_debit_period(self,data, period_id, journal_id=False):
        if journal_id:
            journals = [journal_id]
        else:
        ...
        if data['form']['report_currency']=='base':
            ...
            self.cr.execute('SELECT SUM(l.debit_usd) FROM account_move_line l '
                        'LEFT JOIN account_move am ON (l.move_id=am.id) '
                        'WHERE am.state IN %s AND l.period_id=%s AND l.journal_id IN %s ' + self.query_get_clause + ' ' \
                        'AND l.state<>\'draft\'',
                        (tuple(move_state), period_id, tuple(journals)))
    ...
    def _sum_credit_period(self,data, period_id, journal_id=None):
        if journal_id:
            journals = [journal_id]
        else:
            journals = self.journal_ids
        ...
        if data['form']['report_currency']=='base':
            ...
            self.cr.execute('SELECT SUM(l.credit_usd) FROM account_move_line l '
                        'LEFT JOIN account_move am ON (l.move_id=am.id) '
                        'WHERE am.state IN %s AND l.period_id=%s AND l.journal_id IN %s '+ self.query_get_clause + ' ' \
                        'AND l.state<>\'draft\'',
                        (tuple(move_state), period_id, tuple(journals)))
    ...
    def _get_currency(self, company, data):
        ...
        return res
report_sxw.report_sxw('report.account.general.journal.isa', 'account.journal.period', 'isa_account/report/account_general_journal.rml', parser=journal_print, header='internal')
