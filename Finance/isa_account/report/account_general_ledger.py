...
class general_ledger1(report_sxw.rml_parse, common_report_header):
    _name = 'report.account.general.ledger1'
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        return super(general_ledger1, self).set_context(objects, data, new_ids, report_type=report_type)

    def __init__(self, cr, uid, name, context=None):
        if context is None:
            context = {}
        super(general_ledger1, self).__init__(cr, uid, name, context=context)
        self.query = ""
    ...
    def get_children_accounts(self, account,data):
        res = []
        ...
        for child_account in self.pool.get('account.account').browse(self.cr, self.uid, ids_acc, context=self.context):
            ...
            sold_account = self._sum_balance_account(child_account,data)
    ...
    def lines(self, account,objects,data):
        ...
        if data['form']['report_currency']=='base':
            ...
            sql = """
                SELECT l.id AS lid, l.date AS ldate, j.code AS lcode, l.currency_id,l.amount_currency,l.ref AS lref, l.name AS lname, COALESCE(l.debit_usd,0) AS debit_usd, COALESCE(l.credit_usd,0) AS credit_usd, l.period_id AS lperiod_id, l.partner_id AS lpartner_id,
                m.name AS move_name, m.id AS mmove_id,per.code as period_code,
                c.symbol AS currency_code,
                i.id AS invoice_id, i.type AS invoice_type, i.number AS invoice_number,
                p.name AS partner_name
            ...
            if res_lines and self.init_balance:
                #FIXME: replace the label of lname with a string translatable
                sql = """
                    SELECT 0 AS lid, '' AS ldate, '' AS lcode, COALESCE(SUM(l.amount_currency),0.0) AS amount_currency, '' AS lref, 'Initial Balance' AS lname, COALESCE(SUM(l.debit_usd),0.0) AS debit_usd, COALESCE(SUM(l.credit_usd),0.0) AS credit_usd, '' AS lperiod_id, '' AS lpartner_id,
                    '' AS move_name, '' AS mmove_id, '' AS period_code,
                    '' AS currency_code,
                    NULL AS currency_id,
                    '' AS invoice_id, '' AS invoice_type, '' AS invoice_number,
            ...
            for l in res:
                l['move'] = l['move_name'] != '/' and l['move_name'] or ('*'+str(l['mmove_id']))
                l['partner'] = l['partner_name'] or ''
                account_sum += l['debit_usd'] - l['credit_usd']
                l['progress'] = account_sum
                l['line_corresp'] = l['mmove_id'] == '' and ' ' or counterpart_accounts[l['mmove_id']].replace(', ',',')
                # Modification of amount Currency
                if l['credit_usd'] > 0:
    ...
    def _sum_debit_account(self, account,data):
        ...
        if data['form']['report_currency']=='base':
            ...
            self.cr.execute('SELECT sum(debit_usd) \
                    FROM account_move_line l \
                    JOIN account_move am ON (am.id = l.move_id) \
                    WHERE (l.account_id = %s) \
                    AND (am.state IN %s) \
    ...
    def _sum_credit_account(self, account,data):
        ...
        if data['form']['report_currency']=='base':
            ...
            self.cr.execute('SELECT sum(credit_usd) \
                FROM account_move_line l \
                JOIN account_move am ON (am.id = l.move_id) \
                WHERE (l.account_id = %s) \
                AND (am.state IN %s) \
                ...
                ,(account.id, tuple(move_state)))
            sum_credit = self.cr.fetchone()[0] or 0.0
            #print "______________sum credit_____________________",sum_credit
            if self.init_balance:
                self.cr.execute('SELECT sum(credit_usd) \
                    FROM account_move_line l \
                    JOIN account_move am ON (am.id = l.move_id) \
            ...
            #sum_credit += self.cr.fetchone()[0] or 0.0
            sum_credit+=0.00
        #print "______________sum credit return_____________________",sum_credit
        return sum_credit
    ...
    def _sum_balance_account(self, account,data):
        ...
        if data['form']['report_currency']=='base':
            ...
            self.cr.execute('SELECT (sum(debit_usd) - sum(credit_usd)) as tot_balance \
                FROM account_move_line l \
                JOIN account_move am ON (am.id = l.move_id) \
                WHERE (l.account_id = %s) \
                AND (am.state IN %s) \
                ...
                ,(account.id, tuple(move_state)))
            sum_balance = self.cr.fetchone()[0] or 0.0
           # print " -----------------tota_______amount0000000000000----",sum_balance
            if self.init_balance:
                self.cr.execute('SELECT (sum(debit_usd) - sum(credit_usd)) as tot_balance \
                    FROM account_move_line l \
                    JOIN account_move am ON (am.id = l.move_id) \
            ...
            #sum_balance += self.cr.fetchone()[0] or 0.0
            sum_balance+=0.00
            #print '________________self.cr.fetchone()[0] or 0.0_________',sum_balance
#            
        return sum_balance
    ...
    def _get_account(self, data):
        ...
        return super(general_ledger1 ,self)._get_account(data)
...
report_sxw.report_sxw('report.account.general.ledger1', 'account.account', 'isa_account/report/account_general_ledger11.rml', parser=general_ledger1, header='internal')
report_sxw.report_sxw('report.account.general.ledger_landscape_isa', 'account.account', 'isa_account/report/account_general_ledger_landscape_isa.rml', parser=general_ledger1, header='internal landscape')
