    ...
    def get_lines(self, data):
        ...
        if data['form']['report_currency']=='base':
            ...
            for report in self.pool.get('account.financial.report').browse(self.cr, self.uid, ids2, context=data['form']['used_context']):
                vals = {
                    'name': report.name,
                    'balance': report.balance_usd * report.sign or 0.0,
                    'type': 'report',
                    'level': bool(report.style_overwrite) and report.style_overwrite or report.level,
                    'account_type': report.type =='sum' and 'view' or False, #used to underline the financial report balances
                }
                if data['form']['debit_credit']:
                    vals['debit_usd'] = report.debit_usd
                    vals['credit_usd'] = report.credit_usd
                if data['form']['enable_filter']:
                    vals['balance_cmp'] = self.pool.get('account.financial.report').browse(self.cr, self.uid, report.id, context=data['form']['comparison_context']).balance_usd * report.sign or 0.0
                lines.append(vals)
                account_ids = []
                if report.display_detail == 'no_detail':
                    #the rest of the loop is used to display the details of the financial report, so it's not needed here.
                ...
                if account_ids:
                    for account in account_obj.browse(self.cr, self.uid, account_ids, context=data['form']['used_context']):
                        ...
                        vals = {
                            'name': account.code + ' ' + account.name,
                            'balance':  account.balance_usd != 0 and account.balance_usd * report.sign or account.balance_usd,
                            'type': 'account',
                            'level': report.display_detail == 'detail_with_hierarchy' and min(account.level + 1,6) or 6, #account.level + 1
                            'account_type': account.type,
                        }
                        ...
                        if data['form']['debit_credit']:
                            vals['debit_usd'] = account.debit_usd
                            vals['credit_usd'] = account.credit_usd
                        if not currency_obj.is_zero(self.cr, self.uid, account.company_id.currency_id, vals['balance']):
                            flag = True
                        if data['form']['enable_filter']:
                            vals['balance_cmp'] = account_obj.browse(self.cr, self.uid, account.id, context=data['form']['comparison_context']).balance_usd * report.sign or 0.0
                            if not currency_obj.is_zero(self.cr, self.uid, account.company_id.currency_id, vals['balance_cmp']):
                                flag = True
                        if flag:
                            lines.append(vals)        
...
report_sxw.report_sxw('report.account.financial.report.isa', 'account.financial.report',
    'isa_account/report/account_financial_report_isa.rml', parser=report_account_common, header='internal')
