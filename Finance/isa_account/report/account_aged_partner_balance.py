...
class aged_trial_report(report_sxw.rml_parse, common_report_header):
    ...
    def _get_lines(self, form):
        ...
        if form['report_currency']=='base':
            ...
            self.cr.execute('SELECT l.partner_id, SUM(l.debit_usd-l.credit_usd) \
                        FROM account_move_line AS l, account_account, account_move am \
                        WHERE (l.account_id = account_account.id) AND (l.move_id=am.id) \
                        AND (am.state IN %s)\
                        AND (account_account.type IN %s)\
    ...
    def _get_lines_with_out_partner(self, form):
        ...
        if form['report_currency']=='base':
            ...
            self.cr.execute('SELECT SUM(l.debit_usd-l.credit_usd) \
                        FROM account_move_line AS l, account_account, account_move am \
                        WHERE (l.account_id = account_account.id) AND (l.move_id=am.id)\
                        AND (am.state IN %s)\
                        AND (l.partner_id IS NULL)\
...
report_sxw.report_sxw('report.account.aged_trial_balance.isa', 'res.partner',
        'isa_account/report/account_aged_partner_balance.rml',parser=aged_trial_report, header="internal landscape")
