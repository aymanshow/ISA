    ...
    def import_account_info(self,cr,uid,ids,context=None):
        ...
        for i in range(1,sheet.nrows):
            ...
            if account_code:
                ...
                if account_type:
                    acc_type=account_type_obj.search(cr, uid, [('name','ilike',account_type)], context=context)
...
#                     acc_parent_id=acc_parent_id[0]
                if not acc_id:
                    self.pool.get('account.account').create(cr,uid,{'code' : account_code,'name' : account_name,'portuguese_name':portuguese_name,'type':internal_type,'user_type':acc_type[0],'parent_id':acc_parent_id and acc_parent_id[0] or False}) 
                else:
                     self.pool.get('account.account').write(cr,uid,acc_id,{'code' : account_code,'name' : account_name,'portuguese_name':portuguese_name,'type':internal_type,'user_type':acc_type[0],'parent_id':acc_parent_id[0] and acc_parent_id or False})    
        return True
